...
class UpdateCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'woocommerce:products:update
        {user}
     ...
     * @var string
     */
    protected $description = 'Updates the articles to WooCommerce.';
    ...
    public function handle(BackgroundTasks $BackgroundTasks)
    {
        ...
        $backgroundtask_key = 'user.' . $this->user->id . '.article.woocommerce.products.update';
        ...
        foreach ($this->getArticles() as $article) {
            ...
            echo $articles_count . "\t" . $article->id . "\t" . $article->number . "\t" . $article->card->expansion->abbreviation . "\t" . $article->local_name  . "\t\t\t";
    ...
    {
        $this->user->notify(FlashMessage::success($updated_count .'/' . $articles_count . ' Artikel ' . ($updated_count === 1 ? 'wurde' : 'wurden') . ' zu WooCommerce hochgeladen', [
